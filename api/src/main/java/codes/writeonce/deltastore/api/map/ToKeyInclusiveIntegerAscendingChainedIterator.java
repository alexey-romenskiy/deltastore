...
final class ToKeyInclusiveIntegerAscendingChainedIterator<V, T> extends AbstractIntegerAscendingChainedIterator<V, T> {

    @SuppressWarnings("rawtypes")
    private static final ArrayPool<ToKeyInclusiveIntegerAscendingChainedIterator> POOL =
            new ArrayPool<>(POOL_SIZE, ToKeyInclusiveIntegerAscendingChainedIterator::new);

    private int toKey;

    @SuppressWarnings("unchecked")
    public static <V, T> ToKeyInclusiveIntegerAscendingChainedIterator<V, T> create(
            @Nonnull NestedIterator<V, T> iterator, boolean exclusive, int toKey) {
        final ToKeyInclusiveIntegerAscendingChainedIterator<V, T> value = POOL.get();
    ...
    private ToKeyInclusiveIntegerAscendingChainedIterator() {
        // empty
    }
